...
test("Linux desktop client performs real server-backed login smoke", { skip: process.platform !== "linux" && "Linux-only smoke" }, async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "gvault-linux-login-"));
  ...
  try {
    ...
    await run("dotnet", [
      "publish",
      "apps/desktop/linux/GVault.Desktop.Linux.csproj",
      "-c",
      "Release",
      "-r",
      "linux-x64",
      ...
      "-o",
      "apps/desktop/dist/linux-x64"
    ]);

    const binary = join(root, "apps/desktop/dist/linux-x64/GVault");
    assert.ok(existsSync(binary), "Linux GVault binary exists after publish");

    const output = await run(binary, ["--login-smoke", "--server", base, "--email", email, "--password", password]);
    assert.match(output, /GVault Linux login smoke ok/, "Linux client reports successful server-backed login");

    await assert.rejects(
      () => run(binary, ["--login-smoke", "--server", base, "--email", email, "--password", "wrong-password"]),
      /Login failed with HTTP 401/,
      "Linux client rejects wrong login credentials"
    );
  } finally {
    server.kill();
  }
